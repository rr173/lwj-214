...
export async function createWaitlist(input: CreateWaitlistInput) {
  ...
  if (room.isUnderMaintenance) {
    ...
    if (room.maintenanceStartDate && room.maintenanceStartDate <= today) {
      return {
        success: false,
        errors: [{ field: 'roomNumber', message: `会议室 ${input.roomNumber} 正在维护中，无法候补` }]
      };
    }
    if (room.maintenanceStartDate && input.date >= room.maintenanceStartDate) {
      return {
        success: false,
        errors: [{ field: 'date', message: `会议室 ${input.roomNumber} 将于 ${room.maintenanceStartDate} 起维护，该日期无法候补` }]
      };
    }
  }
